...
void LoadConfig() {
  ...
  if (success)
    ...
    std::cout << "can't read config for some reason so new config file has "
                 "been created"
              << std::endl;
...
void SaveConfig() {
  if (!AimAssist->Save())
    std::cout << "something went wrong trying to save Aimbot settings"
              << std::endl;
  if (!ESP->Save())
    std::cout << "something went wrong trying to save ESP settings"
              << std::endl;
  if (!Trigger->Save())
    std::cout << "something went wrong trying to save Triggerbot settings"
              << std::endl;
  UpdateConfig();
}
// Interface
...
void RenderUI() {
  ...
  ProcessingTimeColor = OverlayWindow.ProcessingTime > 20
                            ? ProcessingTimeColor = ImVec4(1, 0.343, 0.475, 1)
                            : ProcessingTimeColor = ImVec4(0.4, 1, 0.343, 1);
  ImGui::TextColored(ProcessingTimeColor, "Processing Time: %02dms",
                     OverlayWindow.ProcessingTime);
  ImGui::SameLine();
  ...
  if (ImGui::BeginTabItem("Colors", nullptr, ImGuiTabItemFlags_NoCloseWithMiddleMouseButton | ImGuiTabItemFlags_NoReorder)) {
        ImGui::ColorPicker3("Visible Color", VisibleColorTemp, ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_InputRGB);
        ImGui::Separator();

        ImGui::ColorPicker3("Invisible Color", InvisibleColorTemp, ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_InputRGB);
        ImGui::Separator();

        ImGui::ColorPicker3("LockedOn Color", LockedOnColorTemp, ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_InputRGB);
        ImGui::EndTabItem();
    }
    ...
    if (ImGui::BeginTabItem("Extras", nullptr, ImGuiTabItemFlags_NoCloseWithMiddleMouseButton | ImGuiTabItemFlags_NoReorder)) {
      ...
      ImGui::Checkbox("Inv Debug", &inv_debug);
      //ImGui::Checkbox("BunnyHop", &hopelHase);
      ImGui::EndTabItem();
    }
